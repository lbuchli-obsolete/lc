...
reverseState (Lc0State (LArg name expr) env steps) | any (\(n, _) -> n == name) env = reverseState (Lc0State expr env steps)
...
-- TODO prevent name collisions here
